...
export class TerminalGateway implements OnGatewayConnection {
  ...
  handleConnection(client: WebSocket) {
    ...
    client.on('message', (msg: string) => {
      ...
      if (data.type === 'start' && this.windowsClient) {
        this.windowsClient.send(JSON.stringify({ type: 'start' }));
      }

      if (data.type === 'stop' && this.windowsClient) {
        this.windowsClient.send(JSON.stringify({ type: 'stop' }));
      }
